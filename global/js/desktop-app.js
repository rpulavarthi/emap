...
//				holeModifier: "altKey"
	...
	$("#site-attributes-li").click(function(){ 
		...
		$('#csl-nlt-menu').hide();
		$('#geow-menu').hide();
		$('#field-tech-menu').hide();clearFieldTechVectorLayer();active_control_fieldtech = "none_fieldtech";toggleControl_fieldtech();
...
//		viewparams = undefined ;
	...
	$('#geowc0').colorpicker();$('#geowc1').colorpicker(); $('#geowc2').colorpicker(); $('#geowc3').colorpicker();$('#geowc4').colorpicker();$('#geowc5').colorpicker();
...
console.log('mrp 1430 ' + enabledLayer.group + ' - ' + grid_base_url + "?type=neighbors-list&date=" + kpi.date + "&tech=" + kpi.tech + "&techtype=" + kpi.techtype + "&minx=" + bb.bottom + "&miny=" + bb.left + "&maxx=" + bb.top + "&maxy=" + bb.right + ', ' + $(this).attr("group") + ', ' + $(this).attr("group"));
     	   });
     	...
     	// GEOW Menu selected 
             ...
             $("#geow-menu-li").dblclick(function(){
              ...
              $("#geow-submit").click(function(){
                     //get vars
                     var geow = {};
                     geow.date = $("#geow-datepicker").val();
                     geow.hourStart = $("#geow-hour-start").val();
                     geow.hourEnd = $("#geow-hour-end").val();
                     geow.bin = $("#geow-bin-combo").val();
                     geow.paramType = $("#geow-param-combo").val();
                     geow.valType = $("#geow-type").val();

                     geow.color1 = $("#geowv0").val();
                     geow.color2 = $("#geowv1").val();
                     geow.color3 = $("#geowv2").val();
                     geow.color4 = $("#geowv3").val();
                     geow.color5 = $("#geowv4").val();
                     geow.color6 = $("#geowv5").val();

                     $('#layer-list > li').each(function(index) {
                    	 if('LTE' == $(this).attr("layer")) {
                    		 $(this).addClass('active');
                    	 } else {
                    		 $(this).removeClass('active');
                    	 }
                         //var bb = map.getExtent().transform( map.projection, new OpenLayers.Projection("EPSG:4326"));
                         viewparams = "type:geow;zoom:" + map.getZoom() + ";date:" + geow.date + ";hourStart:" + geow.hourStart + ";hourEnd:" + geow.hourEnd + ";bin:" + geow.bin + ";paramType:" + geow.paramType + ";valType:" + geow.valType;
...
//grid_base_url = 'ATT:GEOW';//'http://138.85.245.136/geo/wms/pie' + "?type=geow-list&date=" + kpi.date + "&tw=" + kpi.tw + "&minx=" + bb.bottom + "&miny=" + bb.left + "&maxx=" + bb.top + "&maxy=" + bb.right;
//console.log('2002 ' + map.getZoom());
//console.log('2002 ' + grid_base_url + "?type=geow&date=" + kpi.date + "&tw=" + kpi.tw + "&minx=" + bb.bottom + "&miny=" + bb.left + "&maxx=" + bb.top + "&maxy=" + bb.right);
//console.log('2003 ' + range_url);
//             	    jQuery("#geow-list").jqGrid('setGridParam',{url:'ATT:GEOW' + "?type=geow&date=" + kpi.date + "&tw=" + kpi.tw + "&minx=" + bb.bottom + "&miny=" + bb.left + "&maxx=" + bb.top + "&maxy=" + bb.right}).trigger("reloadGrid");		
                     //alert('todo: toggle layer');
             });
   ...
   $("#kpicd-menu-li").dblclick(function(){
	...
	//    Field Tech Code... will be migrated to a separate js file soon.
	var curredit_geomtype = "";
	var currtitle_ftinput = "";
	var currnotes_ftinput = "";
	var currpoints_ftinput = "";
	var currcascad_ftinput = "";
	var curredit_inputid = "";
	var currfeature_wktgeom = "";
	var curractive_ftinput = "";
    ...
    function report_fieldtech(event)
	{
		...
		{
		    var _wkt = new OpenLayers.Format.WKT();
			currfeature_wktgeom = _wkt.extractGeometry(event.feature.geometry);
            console.log(currfeature_wktgeom);
			curredit_inputid = event.feature.attributes.id;
			$('#ftcInputTitle').val(event.feature.attributes.title);
			$('#ftcInputCascad').val(event.feature.attributes.cascad);
			if(event.feature.attributes.active == 'Yes')
			{$('#active_ftinput').attr("checked", true);}
			else
			{$('#inactive_ftinput').attr("checked", true);}
			$('#ftcInputNotes').val(event.feature.attributes.notes);
			$('#ftcinputModal').modal('show');
			$('#radio_ftinput').buttonset();
		}
	}
...
var _url = "http://evas.ericsson.net/emap/create/ftinput/"+user.client+"/"+user.name+"?title="+currtitle_ftinput+"&notes="+currnotes_ftinput+"&geomtype="+curredit_geomtype+"&points="+currpoints_ftinput+"&cascad="+currcascad_ftinput;
	...
	$("#cancel_ftinput").click(function(){
		clearFieldTechVectorLayer();
		refreshFieldTechVectorLayer();
		$('#ftinputModal').modal('hide');
	})
	
	$("#submit_edit_ftinput").click(function(){
	currnotes_ftinput = $("#ftcInputNotes").val();
	currtitle_ftinput = $("#ftcInputTitle").val();
	currcascad_ftinput = $("#ftcInputCascad").val();
	if($('#active_ftinput').is(':checked'))
	{
		curractive_ftinput = 1;
	}
	else
	{
		curractive_ftinput = 0;
	}
	
	var _url = "http://evas.ericsson.net/emap/modify/ftinput/Sprint-DR?id="+curredit_inputid+"&notes="+currnotes_ftinput+"&title="+currtitle_ftinput+"&cascad="+currcascad_ftinput+"&active="+curractive_ftinput+"&geometry="+currfeature_wktgeom;
	$.ajax({
				  url: _url,
				  context: document.body
				}).done(function() {
		...
		clearFieldTechVectorLayer();
		refreshFieldTechVectorLayer();
		$('#ftcinputModal').modal('hide');
	})
	
	$("#delete_ftinput").click(function(){
	var _url = "http://evas.ericsson.net/emap/delete/ftinput/Sprint-DR?id="+curredit_inputid;
	$.ajax({
		url: _url,
		context:document.body
	}).done(
	function(){
	$( "#dialog-message-deleted" ).dialog({
							  modal: true,
							  buttons: {
								Ok: function() {
								  $( this ).dialog( "close" );
	...
	jQuery("#list_field_tech").jqGrid({
				url:'http://evas.ericsson.net/emap/get/ftinput_datagrid/Sprint-DR',
				datatype: "json",
				height:150,
				colNames:['ID', 'Title', 'Owner', 'Active','Cascad', 'Modify date', 'Notes'],
				colModel: [
				...
				{name:'cascad', index:'cascad', width:100},
				...
				viewrecords: true,
				sortorder: "desc",
				ondblClickRow: function(id){zoomToFeature(id); },
				rowNum:10,
				gridview: true,
				pager:'#pager_field_tech'
				});
				$("#list_field_tech").jqGrid('navGrid','#pager_field_tech',{edit:false,add:false,del:false});
				$('#radio_fieldtech_div_controls').buttonset();
				
	   $.ajax({url:_url}).done(function(data){
			   var _input_array = JSON.parse(data);
			   var _vectorLayer = map.getLayersByName("Field Tech - Vector Layer")[0];
			   var _wkt = new OpenLayers.Format.WKT();
			   ...
			   for(var i =0;i < _input_array.length; i++)
			   {
				 ...
				 _attributes.id = _input_array[i].id;
				 _attributes.cascad = _input_array[i].cascad;
				 _attributes.owner = _input_array[i].owner;
				 if(_input_array[i].active == 1)
				 {
					_attributes.active = 'Yes';
				 }
				 else
				 {
					_attributes.active = 'No';
				 }
				 _attributes.modifydate = _input_array[i].modifydate;
				 _attributes.title = _input_array[i].title;
				 _attributes.notes = _input_array[i].notes;
				 var _features = _wkt.read(_input_array[i].shape);
				 _features.attributes = _attributes;
				  _vectorLayer.addFeatures(_features);
			   }
	...
	{
		var _vectorLayer = map.getLayersByName("Field Tech - Vector Layer")[0];
		var _feature = _vectorLayer.getFeaturesByAttribute("id", id)[0];
		var _bounds = _feature.geometry.bounds;
	...
	$("#noneToggle_fieldtech").click(function(){
		active_control_fieldtech = "none_fieldtech";toggleControl_fieldtech();
	});	
	...
	$("#neighbors-list").jqGrid({
            ...
            colModel :[
                ...
                {name:'CHP', index:'chp', width:50, align:'center'},
                {name:'HOSR', index:'chp', width:50, align:'center'},
	...
	{
		$(this).toggleClass("active");
		togglefeed($(this).attr("url"), $(this).attr("icon"), $(this).attr("name"), $(this).attr("type"), $(this).attr("geom"));
	}
	);
	
	function togglefeed(feed_url,icon,name,feed_type,geom_type)
	{ 
		...
		if(feed_type == "georss")
		{
		var _format = new OpenLayers.Format.GeoRSS();
		var _stylemap = new OpenLayers.StyleMap(feed_style);
		}
		else if(feed_type == "kml")
		{
		var _format = new OpenLayers.Format.KML({
                    extractStyles: true, 
...
//set search url
	...
	function buildKML( layer_name, url){
		...
		if(url.indexOf('138.85.173.7') >= 0) {
			
//			url = url.substring(0, 29) + 'index.php';
		}
	...
	function buildWMS(layer_name, url, options){
		...
		if(url.indexOf('138.85.173.7') >= 0) {
			...
			//url = url.substring(0, 33) + 'index.php';
		}
	...
	else if(typeof layers["T-Mobile"] != 'undefined') {
