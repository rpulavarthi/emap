	...
	function index() {
		...
		foreach($res as $l){
			...
			$layers[$l->client][$l->name][$l->order] = array(
											...
											'search_group' => $l->search_group,
											'pg_key' => $l->pg_key
										);
		}
        // get renderers info
		$renderers = Doo::db()->find('Renderers', array('where' => "client = '". $user["client"] . "'"));
		
		// get disasters info
		$disasters = Doo::db()->find('Disasters', array('where' => "client = '". $user["client"] . "'"));
		
		// get disasters info
		$disaster_categories = Doo::db()->find('Disaster_Categories', array('asc' => 'id'));
		...
		if($dtest_year){
			...
			$sql="SELECT data_source, data_source_name from enod_table_config where year ='". $default_year."' and client = '" . $user["client"] . "' order by data_source_name";
			$dtest_location = Doo::db()->fetchAll($sql);
			
			if($dtest_location){
				$default_ds = $dtest_location[0]['data_source'];
				
				$sql="SELECT query_params_values from enod_table_config where year = '". $default_year."' and data_source='". $default_ds."' and client = '" . $user["client"] . "' order by year";
...
//		$this->renderc('DesktopMain', array('layers' => $layers, 'user' => $user, 'links' => $links, 'clients' => $clients));
		$this->renderc('DesktopMain', array('layers' => $layers, 'renderers' => $renderers,'disasters' => $disasters,'disaster_categories' => $disaster_categories, 'user' => $user, 'links' => $links, 'clients' => $clients, 'dtest_year' => $dtest_year, 'dtest_location' => $dtest_location, 'dtest_params' => $dtest_params));
	}
